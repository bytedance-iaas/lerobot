...
run it, start a local dev server and point the test at it::
    ...
    LEROBOT_LIVEKIT_URL=ws://127.0.0.1:7880 \
        ...
        pytest tests/robots/test_webrtc_proxy_livekit.py -p no:hydra_pytest -sv

LiveKit's FFI client binds one room per process, so the Mac daemon (publisher) runs as
...
@pytest.fixture
def daemon():
    """Run the Mac daemon (publisher) as its own process, streaming a synthetic source."""
    proc = subprocess.Popen(
        [
            sys.executable,
            "-m",
            "lerobot.robots.webrtc_proxy.mac_daemon",
            "--session",
            _ROOM,
            "--transport",
            "livekit",
