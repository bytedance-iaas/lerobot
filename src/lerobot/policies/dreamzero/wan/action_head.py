...
# Ported from NVIDIA DreamZero: groot/vla/model/dreamzero/action_head/wan_flow_matching_action_tf.py
# Surgical changes for the lerobot port (behaviour preserved):
#   - drop hydra `instantiate` / `accelerate` (unused) imports; build sub-modules directly
#   - `WANPolicyHeadConfig` is a plain dataclass instead of transformers `PretrainedConfig`
#   - `groot.vla...` imports rewritten to relative `.` imports; `ActionHead` from local base
#   - TensorRT engine path (LOAD_TRT_ENGINE) raises instead of importing groot.control
...
from .vram import enable_vram_management, AutoWrappedModule, AutoWrappedLinear
from .text_encoder import T5RelativeEmbedding, T5LayerNorm, WanTextEncoder
from .schedulers import FlowUniPCMultistepScheduler
from .image_encoder import WanImageEncoder
from .vae import WanVideoVAE, WanVideoVAE38
from .causal_wan_model import CausalWanModel


def _component_kwargs(cfg):
    """Return constructor kwargs from a component config dict, dropping hydra-style meta keys."""
...
class WANPolicyHeadConfig:
    ...
    # Which VAE class to build ("WanVideoVAE" for Wan2.1/16ch, "WanVideoVAE38" for Wan2.2/48ch).
    # Replaces the hydra `_target_` selection that used to live in vae_cfg.
    vae_class: str = field(default="WanVideoVAE")
...
class WANPolicyHead(ActionHead):
    ...
    def __init__(
        ...
        self.text_encoder = WanTextEncoder(**_component_kwargs(config.text_encoder_cfg))
        self.image_encoder = WanImageEncoder(**_component_kwargs(config.image_encoder_cfg))
        vae_cls = WanVideoVAE38 if getattr(config, "vae_class", "WanVideoVAE") == "WanVideoVAE38" else WanVideoVAE
        self.vae = vae_cls(**_component_kwargs(config.vae_cfg))
        ...
        self.model = CausalWanModel(**_component_kwargs(config.diffusion_model_cfg))
    ...
    def post_initialize(self):
        ...
        if LOAD_TRT_ENGINE is not None:
            raise RuntimeError(
                "TensorRT engine loading (LOAD_TRT_ENGINE) is not supported in the lerobot DreamZero port; "
                "groot.control.tensorrt_utils was not ported."
            )
