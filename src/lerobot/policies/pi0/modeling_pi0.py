...
def compute_layer_complete(inputs_embeds, attention_mask, position_ids, adarms_cond, layers, rotary_emb):
    ...
    for i, hidden_states in enumerate(inputs_embeds):
        ...
        if getattr(layer.mlp, "absorbs_post_attention_layernorm", False):
            ...
            # straight into the fused MLP. adarms_cond is None for the VLM branch, so the norm
            # gate is None and the second residual becomes a plain add.
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            # history + scale). A bf16 checkpoint has none, so with fp8 enabled those
            # `...layernorm_mlp._extra_state` keys are EXPECTED to be missing — TE inits them fresh,
            # which is correct when starting fp8 from a bf16 checkpoint (scales calibrate during
            # training). Not a load error. (strict=True previously RAISED on these; the outer
            # except then swallowed it as a scary "Could not load state dict" — which would have
            # hidden a genuinely missing weight the exact same way.)
