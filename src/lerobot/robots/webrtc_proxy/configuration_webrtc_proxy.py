...
class WebRTCProxyRobotConfig(RobotConfig):
    """Cloud-side proxy: real hardware lives on the user's Mac, reached over WebRTC.
    ...
    # Schema (must match the Mac-side real robot exactly so dataset/policy shapes line up).
    ...
    capture_fps: int = 30
    # P0 safety: Mac watchdog safes the arm if no action arrives within this window.
